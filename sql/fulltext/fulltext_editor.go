...
func (editor TableEditor) Insert(ctx *sql.Context, row sql.Row) error {
	...
	for _, index := range editor.Indexes {
		...
		for ; err == nil && !reachedTheEnd; word, position, reachedTheEnd, err = parser.Next(ctx) {
			...
			positionRow := sql.NewSqlRowWithLen(len(SchemaPosition) + len(keyCols))
			positionRow.SetValue(0, word)
			for i := 1; i < len(keyCols); i++ {
				positionRow.SetValue(i, keyCols[i])
			}
		...
		for ; err == nil && !reachedTheEnd; word, reachedTheEnd, err = parser.NextUnique(ctx) {
			...
			docCountRow := sql.NewSqlRowWithLen(len(SchemaPosition) + len(keyCols))
			docCountRow.SetValue(0, word)
			for i := 1; i < len(keyCols); i++ {
				docCountRow.SetValue(i, keyCols[i])
			}
